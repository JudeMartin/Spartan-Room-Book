...
public class ReservationDAOImpl implements ReservationDAO {
	...
	public Integer checkAvailability(SearchCriteria searchCriteria) {
		...
		String queryString = "from Reservation r "
				+ "right outer join r.room where r.room.roomTypeId=:roomTypeId1 "
				+ "and r.room.id not in "
				+ "(select res.room.id from Reservation as res where "
		...
		//String queryString2 = "from Reservation r right outer join r.room";//where check_in_date > :code
		//String queryString2 = "select Room.id from Room inner join Reservation r on r.room";
		...
		/*if (list.size() > 0){
			//return any one room id to the user
			Reservation reservationObj = (Reservation)list.get(0);
			int roomId = reservationObj.getRoom().getId().intValue();
			return roomId;
		}
		else*/
			return 0;
